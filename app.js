...
const SESSION_KEY = 'tacam_session';
...
function updateAppVisibility() {
  ...
  if (sessionUser) {
    sessionUser.textContent = loggedIn ? `Sesión: ${currentUser}` : '';
  }
}

function login(username, password) {
  if (ADMIN_USERS[username] !== password) return false;
...
function renderVersion() {
  if (appVersion) {
    appVersion.textContent = `Versión visible en pantalla principal: ${APP_VERSION}`;
  }
}
