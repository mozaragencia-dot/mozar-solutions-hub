...
function applyRoleAccess() {
  ...
  const allowedForLawyer = new Set(['prison-visits', 'imputados', 'clients', 'lawyers']);
  moduleTabs.forEach(tab => {
    if (role !== 'Abogada') {
      tab.hidden = false;
      return;
...
sendGendarmeriaEmailBtn.addEventListener('click', async () => {
  ...
  if (!visits.length) {
    showToast('No hay visitas para enviar en el filtro actual.');
    return;
  }
  ...
  try {
    ...
    console.error('No se pudo enviar nómina a Gendarmería', error);
    showToast('Error al enviar nómina a Gendarmería.');
  }
});
